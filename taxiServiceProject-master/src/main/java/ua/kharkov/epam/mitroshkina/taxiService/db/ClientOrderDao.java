...
public class ClientOrderDao {
    ...
    private static final String SORT_CATEGORIES = "SELECT cat.category_name, COUNT(*) as count FROM confirmedOrder co" +
    ...
    private final static String SORT_ORDERS_BY_DATE_DESC= "SELECT cono.confirmedOrder_id, clo.user_id, dep.str_name, des.str_name, clo.passengers_number, c.car_id, cono.cost, cono.discount_cost, clo.order_date, cono.status\n" +
            "FROM addresses dep, addresses des, clientOrder clo, car_category ca, car_description d, cars c, confirmedOrder cono\n" +
    ...
    private final static String SORT_ORDERS_BY_DATE_ASC= "SELECT cono.confirmedOrder_id, clo.user_id, dep.str_name, des.str_name, clo.passengers_number, c.car_id, cono.cost, cono.discount_cost, clo.order_date, cono.status\n" +
            "FROM addresses dep, addresses des, clientOrder clo, car_category ca, car_description d, cars c, confirmedOrder cono\n" +
            "WHERE cono.clientOrder_id = clo.id AND clo.car_category = ca.category_id AND dep.address_id = clo.departure_address AND des.address_id = clo.destination_address AND cono.car_id = c.car_id AND c.car_description_id = d.car_description_id ORDER BY clo.order_date ASC";
    ...
    public List<ConfirmedOrderBean> sortConfirmedOrderBeansByDateDESC() throws SQLException {
        try (Connection conn = getConnection()) {
            return sortConfirmedOrderBeansByDateDESC(conn);
        } catch (SQLException e) {
            System.err.println(e.getMessage());
            throw e;
        }
    ...
    private List<ConfirmedOrderBean> sortConfirmedOrderBeansByDateDESC(Connection conn) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(SORT_ORDERS_BY_DATE_DESC);
             ResultSet rs = ps.executeQuery()) {
            List<ConfirmedOrderBean> list = new LinkedList<>();
            while (rs.next()) {
                ConfirmedOrderBean bean = getConfirmedOrderBeanValueForAdmin(rs);
    ...
    public List<ConfirmedOrderBean> sortConfirmedOrderBeansByDateASC() throws SQLException {
        try (Connection conn = getConnection()) {
            return sortConfirmedOrderBeansByDateASC(conn);
        } catch (SQLException e) {
            System.err.println(e.getMessage());
            throw e;
        }
    ...
    private List<ConfirmedOrderBean> sortConfirmedOrderBeansByDateASC(Connection conn) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(SORT_ORDERS_BY_DATE_ASC);
             ResultSet rs = ps.executeQuery()) {
            List<ConfirmedOrderBean> list = new LinkedList<>();
            while (rs.next()) {
                ConfirmedOrderBean bean = getConfirmedOrderBeanValueForAdmin(rs);
    ...
    public List<CategoryBean> sortCategories() throws SQLException {
        try (Connection conn = getConnection()) {
            return sortCategory(conn);
        } catch (SQLException e) {
            System.err.println(e.getMessage());
            throw e;
        }
    ...
    private List<CategoryBean> sortCategory(Connection conn) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(SORT_CATEGORIES);
             ResultSet rs = ps.executeQuery()) {
            List<CategoryBean> list = new LinkedList<>();
            while (rs.next()) {
                CategoryBean bean = getCategoryBeanValue(rs);
                list.add(bean);
            }
            return list;
        }
    ...
    public List<ConfirmedOrderBean> getOrdersByUser(int user_id) throws SQLException {
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        List<ConfirmedOrderBean> list = new LinkedList<>();
        try {
            con = getConnection();
            ps = con.prepareStatement(GET_ORDERS_BY_USER);
            ps.setInt(1, user_id);
            rs = ps.executeQuery();
            while (rs.next()) {
                ConfirmedOrderBean bean = getConfirmedOrderBeanValue(rs);
                list.add(bean);
            }
        } catch (SQLException e) {
            System.err.println(e.getMessage());
    ...
    private ConfirmedOrderBean getConfirmedOrderBeanValue(ResultSet rs) throws SQLException{
        try {
            ...
            cob.setCost(rs.getInt("cost"));
            cob.setDiscount_cost(rs.getInt("discount_cost"));
            cob.setOrder_date("order_date");
            cob.setStatus("status");
            return cob;
        } catch (SQLException e) {
            throw new IllegalStateException(e);
        }
    ...
    private CategoryBean getCategoryBeanValue(ResultSet rs) throws SQLException{
        try {
            CategoryBean cb = new CategoryBean();
            cb.setCategory_name(rs.getString("cat.category_name"));
